...
class Async_R_MAPPO_Lagr(R_MAPPO_Lagr):
    ...
    def cal_cost_value_loss(self, cost_values, cost_preds_batch, cost_return_batch, active_masks_batch):
        ...
        cost_pred_clipped = cost_preds_batch + (cost_values - cost_preds_batch).clamp(-self.clip_param,
                                                                                      self.clip_param)
        if self._use_popart or self._use_valuenorm:
            cost_error_clipped = self.cost_value_normalizer(cost_return_batch) - cost_pred_clipped
            cost_error_original = self.cost_value_normalizer(cost_return_batch) - cost_values
        else:
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        adv_targ, available_actions_batch, factor_batch, cost_preds_batch, cost_return_batch, rnn_states_cost_batch, \
