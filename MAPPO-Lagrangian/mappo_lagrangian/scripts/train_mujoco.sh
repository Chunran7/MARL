...
do
    ...
    if [ "$use_async" = true ]; then
        # 异步训练命令
        CUDA_VISIBLE_DEVICES=0 python train/train_mujoco_async.py  --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} --scenario ${scenario} --agent_conf ${agent_conf} --agent_obsk ${agent_obsk} --lr 9e-5 --critic_lr 5e-3 --std_x_coef 1 --std_y_coef 5e-1 --seed ${seed_} --n_training_threads 4 --n_rollout_threads 16 --num_mini_batch 40 --episode_length 1000 --num_env_steps 10000000 --ppo_epoch 5 --use_value_active_masks  --add_center_xy --use_state_agent  --safety_bound 0.2 --lamda_lagr 0.78 --lagrangian_coef_rate 1e-7 --async_mode ${async_mode} --selection_ratio ${selection_ratio}
    else
        # 原始训练命令
